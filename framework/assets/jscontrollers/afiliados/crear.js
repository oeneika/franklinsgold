 ...
 * Ajax action to api rest
*/
function crear_intercambio(){
    var $ocrendForm = $(this), __data = {};
    $('#crear_intercambio_form').serializeArray().map(function(x){__data[x.name] = x.value;}); 

    if(undefined == $ocrendForm.data('locked') || false == $ocrendForm.data('locked')) {
        $('#crearintercambiobtn').attr('disabled','disabled');
        $.ajax({
            type : "POST",
            url : "api/afiliados/crearIntercambio",
            dataType: 'json',
            data : __data,
            beforeSend: function(){ 
                $ocrendForm.data('locked', true) 
            ...
            complete: function(){ 
                $('#crearintercambiobtn').removeAttr('disabled');
                $ocrendForm.data('locked', false);
            } 
        });
    }
...
$('#crearintercambiobtn').click(function(e) {
    e.defaultPrevented;
    crear_intercambio();
});
$('form#crear_intercambio_form input').keypress(function(e) {
    e.defaultPrevented;
    if(e.which == 13) {
        crear_intercambio();
